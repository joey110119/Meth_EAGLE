...
#include "calc.h"
//#include "calc_gpu.h"
#define print_status_rni(M, ...)  fprintf(stderr, M, ##__VA_ARGS__);
...
void calc_prob_snps_region(double *prgu, double *prgv, vector_int_t *combo, variant_t **var_data, double *matrix, int read_length, const char *seq,  \
int seq_length, int pos, int start, int end, int *seqnt_map) {
    if (start < 0) start = 0;
...
void calc_prob_snps(double *prgu, double *prgv, vector_int_t *combo, variant_t **var_data, double *matrix, int read_length, const char *seq, int seq_length, \
int pos, int *splice_pos, int *splice_offset, int n_splice, int *seqnt_map) {
    /* Get the sequence g in G and its neighborhood (half a read length flanking regions) */
    int start = pos - (read_length / 2);
    int end = pos + (read_length / 2);
