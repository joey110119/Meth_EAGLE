...
/* eagle_meth */
#define METH_EAGLE 1
char* correct_ref = NULL; // the-highest-likelihood-variant-modified Ref version
int contruct_already = 0; // in order not to re-assign value to "char* correct_ref" again
int correct_ref_length; // the length of "char* correct_ref" (as the parameter out in construct function)
char * forward_correct_ref = NULL; // do CtoT without CpG on correct_ref
char * reverse_correct_ref = NULL; // do GtoA without GpC on correct_ref
char CorG;


/* for experimant test */
char* unMe_ref = NULL;
int unMe_contruct_already = 0;
int unMe_ref_length; 
...
static vector_t *bam_fetch_meth(const char *bam_file, const char *chr, const int pos1, const int pos2) {
    /* Reads in region coordinates */
    vector_t *read_list = vector_create(64, READ_T);
    ...
    if (iter != NULL) {
        ...
        while (sam_itr_next(sam_in, iter, aln) >= 0) {
            read_t *read = read_fetch(bam_header, aln, pao, isc, nodup, splice, phred64, const_qual);
            if (read != NULL) {
                // char CorG: split the forward / reverse ref
                if(CorG=='C' && read->is_reverse==0){
                    print_status("forward add\n");
                    vector_add(read_list, read);
                }
                else if(CorG=='G' && read->is_reverse==1){
                    print_status("reverse add\n");
                    vector_add(read_list, read);
                }
            }
        }
...
static void calc_likelihood(stats_t *stat, vector_t *var_set, const char *refseq, const int refseq_length, read_t **read_data, const int nreads, int seti, int *seqnt_map) {
    ...
    for (readi = 0; readi < nreads; readi++) {
        ...
        double prgu, prgv;
        // double prguct, prgvct;// record the prob of  different seq("ref+C to T" without CG) 
        // double prguga, prgvga;// record the prob of  different seq("ref+G to A" without GC) 
        ...
        if (dp) {
            ...
            // different seq("ref+C to T" without CG), different seq("ref+G to A" without GC)  
            /*double * methlevel_ct = (double *)malloc(sizeof(double) * (refseq_length));
            double * methlevel_ga = (double *)malloc(sizeof(double) * (refseq_length));
            char * refseq_ct = (char *)malloc(sizeof(char) * (refseq_length+1));
            char * refseq_ga = (char *)malloc(sizeof(char) * (refseq_length+1));
            int index_ct = 0, index_ga = 0;
            for(int i=0; i<refseq_length; i++){
                if(refseq[i] == 'C' || refseq[i]=='c'){
                    ...
                    refseq_ct[i] = refseq[i];
                    methlevel_ct[i] = 0;
                    refseq_ga[i] = refseq[i];
                    methlevel_ga[i] = 0;
                }
            }
            strcpy(refseq_ct, refseq);
            strcpy(refseq_ga, refseq);
            calc_prob_snps(&prguct, &prgvct, stat->combo, var_data, readprobmatrix, read_data[readi]->length, refseq_ct, refseq_length, read_data[readi]->pos, read_data[readi]->splice_pos, read_data[readi]->splice_offset, read_data[readi]->n_splice, seqnt_map);
            calc_prob_snps(&prguga, &prgvga, stat->combo, var_data, readprobmatrix, read_data[readi]->length, refseq_ga, refseq_length, read_data[readi]->pos, read_data[readi]->splice_pos, read_data[readi]->splice_offset, read_data[readi]->n_splice, seqnt_map);
            
            // normal seq("original refernece")
            calc_prob_snps(&prgu, &prgv, stat->combo, var_data, readprobmatrix, read_data[readi]->length, refseq, refseq_length, read_data[readi]->pos, read_data[readi]->splice_pos, read_data[readi]->splice_offset, read_data[readi]->n_splice, seqnt_map);
        
            // the sum prob
            prgu = prgu + prguct + prguga;
            prgu = prgv + prgvct + prgvga;*/
        }
        //printf("%f\t%f\n\n", prgv, prgu);
...
static char *evaluate(vector_t *var_set) {
    ...
    */
    ...
    if (mvh) { /* Max likelihood variant hypothesis */
        ...
        print_status("total: %d\tmax_seti %ld\n",all_rni, max_seti);
        ...
        for (i = 0; i < stat[max_seti]->combo->len; i++) {
            ...
            if(contruct_already ==0){
                correct_ref = strdup(refseq);
                correct_ref_length = refseq_length;
                contruct_already = 1;
            }
            int altseq_length;
            //print_status("%s", correct_ref);
            correct_ref = construct_altseq(correct_ref, correct_ref_length, (vector_int_t *)stat[max_seti]->combo, var_data, &altseq_length);
            correct_ref_length = altseq_length;
        }
        variant_print(&output, v, 0, stat[max_seti]->seen, stat[max_seti]->ref_count, stat[max_seti]->alt_count, log_add_exp(total, stat[max_seti]->ref), has_alt, stat[max_seti]->ref);
        vector_free(v); //variants in var_list so don't destroy
    }
...
vector_t * ctga_varlist_create(char *correct_ref, char CorG){
    vector_t *ctga_var_list = vector_create(8, VARIANT_T);
    print_status("enter ctga_varlist_create(), the ref len is %d\n", strlen(correct_ref));
    for(int i=0; i<strlen(correct_ref); i++){
        //print_status("i is %d\n", i);
        if(correct_ref[i]=='c' || correct_ref[i]=='C' || correct_ref[i]=='g' || correct_ref[i]=='G'){
            int pos = i+1; // position is started from 1, but this for loop is started from 0, so plus 1
            char *chr, *ref, *alt;
            chr = strdup("test"); // chr name
            if((correct_ref[i]=='c' || correct_ref[i]=='C') && CorG=='C'){
                ref = strdup("C");
                alt = strdup("T");
                variant_t *v = variant_create(chr, pos, ref, alt);
                vector_add(ctga_var_list, v);
            }
            else if((correct_ref[i]=='g' || correct_ref[i]=='G') && CorG=='G'){
                ref = strdup("G");
                alt = strdup("A");
                variant_t *v = variant_create(chr, pos, ref, alt);
                vector_add(ctga_var_list, v);
    ...
    return ctga_var_list;
}

static char *evaluate_meth(vector_t *var_set) {
    size_t i, readi, seti;
    ...
    char *refseq;
    if(CorG == 'C'){
        refseq = forward_correct_ref;
    }
    else if(CorG == 'G'){
        refseq = reverse_correct_ref;
    }
    int refseq_length = correct_ref_length;
    ...
    */
    ...
    if (test){
        ...
        for (i = 0; i < stat[max_seti]->combo->len; i++) {
            ...
            if(unMe_contruct_already ==0){
                unMe_ref = strdup(correct_ref);
                unMe_ref_length = correct_ref_length;
                unMe_contruct_already = 1;
            }
            int altseq_length;
            //print_status("%s", correct_ref);
            unMe_ref = construct_altseq(unMe_ref, unMe_ref_length, (vector_int_t *)stat[max_seti]->combo, var_data, &altseq_length);
            unMe_ref_length = altseq_length;
        }
        print_status("\nfor experient test:::::\n")
        print_status("%s\n\n", unMe_ref);
        variant_print(&output, v, 0, stat[max_seti]->seen, stat[max_seti]->ref_count, stat[max_seti]->alt_count, log_add_exp(total, stat[max_seti]->ref), has_alt, stat[max_seti]->ref);
...
static void *pool_meth(void *work) {
    print_status("enter pool_meth()\n");
    work_t *w = (work_t *)work;
    
    print_status("CorG is %c\n", CorG);

    size_t n = w->len / 10;
    print_status("w->len is %d\n", w->len);
    while (1) { //pthread_t ptid = pthread_self(); uint64_t threadid = 0; memcpy(&threadid, &ptid, min(sizeof (threadid), sizeof (ptid)));
        pthread_mutex_lock(&w->q_lock);
        vector_t *var_set = (vector_t *)vector_pop(w->queue);
        //print_status("chr\t%s\n", ((variant_t *)(var_set->data[0]))->chr);
        pthread_mutex_unlock(&w->q_lock);
        if (var_set == NULL) {
        ...
        char *outstr = evaluate_meth(var_set);
        if (outstr != NULL) {
            pthread_mutex_lock(&w->r_lock);
            if (!verbose && n > 10 && w->results->len > 10 && w->results->len % n == 0) {
                print_status("# Progress: %zd%%: %zd / %zd\t%s", 10 * w->results->len / n, w->results->len, w->queue->len, asctime(time_info));
...
static void process_meth(const vector_t *var_list, FILE *out_fh) {
    ...
    pthread_t tid[nthread];
    for (i = 0; i < nthread; i++) pthread_create(&tid[i], NULL, pool_meth, w);
    for (i = 0; i < nthread; i++) pthread_join(tid[i], NULL);
...
int main(int argc, char **argv) {
    ...
    if(METH_EAGLE){
        print_status("correct_ref %s\n", correct_ref);

        forward_correct_ref = strdup(correct_ref);
        reverse_correct_ref = strdup(correct_ref);
        for(int i=0; i<correct_ref_length; i++){
            if(i==correct_ref_length-1){
                if(forward_correct_ref[i]=='C'){
                    forward_correct_ref[i]='T';
                    break;
                }
            }
            // assume whole C without CpG is unMe, so C to T
            if(forward_correct_ref[i]=='C' && forward_correct_ref[i+1]!='G'){
                forward_correct_ref[i]='T';
            }
        }
        for(int i=0; i<correct_ref_length; i++){
            if(i==0){
                if(reverse_correct_ref[i]=='G'){
                    reverse_correct_ref[i]='A';
                    continue;
                }
            }
            // assume whole G without GpC is unMe, so G to A
            if(reverse_correct_ref[i]=='G' && reverse_correct_ref[i-1]!='C'){
                reverse_correct_ref[i]='A';
            }
        }

        print_status("forward_correct_ref %s\n", forward_correct_ref);
        print_status("reverse_correct_ref %s\n", reverse_correct_ref);

        vector_t * ct_var_list = ctga_varlist_create(forward_correct_ref, 'C');
        vector_t * ga_var_list = ctga_varlist_create(reverse_correct_ref, 'G');

        print_status("ct & ga, 2 var_list is done\n");
        print_status("ct_var_list len is %d\n", ct_var_list->len);
        print_status("ga_var_list len is %d\n", ga_var_list->len);
        // for (int j = 0; j < ctga_var_list->len; j++){
        //     print_status("ctga_var_list[%d]\n", j);
        //     print_status("chr\t%s\n", ((variant_t *)(ctga_var_list->data[j]))->chr);
        //     print_status("pos\t%d\n", ((variant_t *)(ctga_var_list->data[j]))->pos);
        //     print_status("ref\t%s\n", ((variant_t *)(ctga_var_list->data[j]))->ref) ;
        //     print_status("alt\t%s\n", ((variant_t *)(ctga_var_list->data[j]))->alt);
        // }


        out_fh = fopen("output_meth_CT.tab", "w");
        pthread_mutex_init(&refseq_lock, NULL);
        CorG = 'C';
        process_meth(ct_var_list, out_fh);
        if (out_file != NULL) fclose(out_fh);
        else fflush(stdout);
        pthread_mutex_destroy(&refseq_lock);

        out_fh = fopen("output_meth_GA.tab", "w");
        pthread_mutex_init(&refseq_lock, NULL);
        CorG = 'G';
        process_meth(ga_var_list, out_fh);
        if (out_file != NULL) fclose(out_fh);
        else fflush(stdout);
        pthread_mutex_destroy(&refseq_lock);
    }
